import customtkinter as ctk  # Ensure customtkinter is correctly installed
...
class Application(ctk.CTk):
    def __init__(self):
        ...
        self.expense_button = ctk.CTkButton(self.sidebar_frame, text="Expenses", command=self.expenses_button_event)
        self.expense_button.grid(row=2, column=0, padx=20, pady=20)
        ...
        # Create entry forms (User can type text)
        ...
        self.expense_frame, self.expense_tree = self.create_transaction_frame("Expense", self.add_expenses, row=2, transactions=self.expense_transactions)
    ...
    def add_expenses(self, category_entry, amount_entry):
        amount = self.get_amount_from_entry(amount_entry)
        category = category_entry.get()
        if amount is not None:
            self.expense += amount
    ...
    def update_plot(self):
        self.fig.clear()
        ax = self.fig.add_subplot(111)  # Create a new subplot
        ax.pie([self.income, self.expense], labels=['Income', 'Expenses'], autopct='%1.1f%%')
        self.canvas.draw()
